...
export const RoleBasedNavigation = () => {
  ...
  return (
    <div className="glass-card rounded-lg shadow-md p-6 mb-6">
      <h3 className="text-lg font-semibold text-white/90 mb-4">Your Access Level</h3>
      ...
      <div className="flex items-center space-x-3 mb-4">
        ...
        <span className="text-sm text-white/60">• {permissions.length} permissions</span>
      </div>
      
      <div className="space-y-2">
        <p className="text-sm text-white/60">You have access to:</p>
        <ul className="text-sm text-white/70 space-y-1">
          {permissions.map((permission) => (
            <li key={permission.name} className="flex items-center space-x-2">
              <span className="w-2 h-2 bg-green-400 rounded-full"></span>
              <span>{permission.name}</span>
              <span className="text-white/50">- {permission.description}</span>
            </li>
          ))}
        </ul>
      </div>
